...
if !exists("g:gt_autodismiss_on_select")
	let g:gt_autodismiss_on_select = 1
endif
...
if exists("g:buffergator_split_size")
	...
	if !exists("g:buffergator_vsplit_size")
		let g:buffergator_vsplit_size = &columns * 2 / 3
	endif
	if !exists("g:h_buffergator_hsplit_size")
		let g:buffergator_hsplit_size =  &lines * 2 / 3
	endif
endif
...
" ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
let s:buffergator_catalog_sort_regimes =
	\ ['basename', 'filepath', 'extension', 'bufnum', 'mru']
...
" ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
let s:buffergator_catalog_display_regimes =
	\ ['basename', 'filepath', 'bufname']
...
function! s:_format_truncated(str, max_len, trunc)
	if len(a:str) > a:max_len
		...
		elseif a:trunc < 0
			return '... '
				\ . strpart(a:str, len(a:str) - a:max_len + 4)
		endif
	else
		return a:str
	endif
...
function! s:_format_filled(str, width, align, trunc)
	...
	if a:trunc != 0
		let l:prepped =
			\ s:_format_truncated(a:str, a:width, a:trunc)
	endif
	if len(l:prepped) < a:width
		if a:align > 0
			let l:prepped =
				\ s:_format_align_right(l:prepped, a:width, " ")
		elseif a:align < 0
			let l:prepped =
				\ s:_format_align_left(l:prepped, a:width, " ")
		endif
	endif
	return l:prepped
endfunction
...
function! s:NewMessenger(name)
	...
	if empty(a:name)
		...
		let l:messenger["title"] =
			\ "buffergator (" . l:messenger["name"] . ")"
	endif
...
function! s:_get_split_mode()
	if has_key(s:buffergator_viewport_split_modes,
		\ g:buffergator_viewport_split_policy)
		return s:buffergator_viewport_split_modes[
			\ g:buffergator_viewport_split_policy]
	else
		call s:_messenger.send_error(
			\ "Unrecognized split mode specified by
			\ 'g:buffergator_viewport_split_policy': "
			\ . g:buffergator_viewport_split_policy)
	endif
endfunction
...
function! s:_find_mru_bufnr(dir)
	...
	if l:target_buf_idx < 0
		if g:buffergator_mru_cycle_loop
			...
			call s:_messenger.send_info(
				\ "already at most recent buffer")
			return -1
		endif
...
function! s:NewCatalogViewer(name, title)
	...
	let l:buffergator_bufs =
		\ s:_find_buffers_with_var("is_gt_buffer", 1)
	...
	function! l:catalog_viewer.line_symbols(bufinfo) dict
	  ...
	  for l:status in l:noted_status
		if a:bufinfo['is_' . l:status]
		  let l:line_symbols .=
			  \ s:buffergator_buffer_line_symbols[l:status]
		else
		  let l:line_symbols .= " "
		endif
	  endfor
	...
	function! l:catalog_viewer.list_buffers() dict
		...
		for l:buffers_output_row in l:buffers_output_rows
			let l:parts = matchlist(
				\ l:buffers_output_row, '^\s*\(\d\+\)\(.....\) "\(.*\)"')
			let l:info = {}
	...
	function! l:catalog_viewer.activate_viewport() dict
		...
		elseif l:bfwn >= 0
			...
			execute("silent keepalt keepjumps "
				\ . self.split_mode . " " . self.bufnum)
			if g:buffergator_viewport_split_policy
				\ =~ '[RrLl]' && g:buffergator_vsplit_size
				execute("vertical resize " . g:buffergator_vsplit_size)
				setlocal winfixwidth
			elseif g:buffergator_viewport_split_policy
				\ =~ '[TtBb]' && g:buffergator_hsplit_size
	...
	function! l:catalog_viewer.claim_buffer() dict
		call setbufvar("%", "is_gt_buffer", 1)
		call setbufvar("%", "gt_catalog_viewer", self)
		call setbufvar("%", "gt_last_render_time", 0)
		call setbufvar("%", "gt_cur_line", 0)
	endfunction

	" 'Unclaims' a buffer by stripping all buffergator vars
	function! l:catalog_viewer.unclaim_buffer() dict
		for l:var in ["is_gt_buffer",
					\ "gt_catalog_viewer",
					\ "gt_last_render_time",
					\ "gt_cur_line"
					\ ]
	...
	function! l:catalog_viewer.setup_buffer_commands() dict
		" command! -bang -nargs=* Bdfilter :call b:gt_catalog_viewer.set_filter('<bang>', <q-args>)
		augroup BuffergatorCatalogViewer
			au!
			autocmd BufLeave <buffer>
				\ let s:_gt_last_catalog_viewed
				\ = b:gt_catalog_viewer
		augroup END
	endfunction
	...
	function! l:catalog_viewer.expand_screen() dict
		if has("gui_running") && g:buffergator_autoexpand_on_split
			if g:buffergator_viewport_split_policy
				\ =~ '[RL]' && g:buffergator_vsplit_size
				let self.pre_expand_columns = &columns
				let &columns += g:buffergator_vsplit_size
				let self.columns_expanded =
					\ &columns - self.pre_expand_columns
			else
				let self.columns_expanded = 0
			endif
			if g:buffergator_viewport_split_policy
				\ =~ '[TB]' && g:buffergator_hsplit_size
	...
	function! l:catalog_viewer.is_usable_viewport(winnumber) dict
		...
		return !modified || self.num_viewports_on_buffer(
			\ winbufnr(a:winnumber)) >= 2
	endfunction
	...
	function! l:catalog_viewer.goto_pattern(
		\ pattern, direction) dict range
		...
		if l:lnum < 0
			if l:flags[0] == "b"
				call s:_messenger.send_info("No previous results")
			else
				call s:_messenger.send_info("No more results")
			endif
			return 0
		else
			return 1
	...
	function! l:catalog_viewer.cycle_sort_regime() dict
		let l:cur_regime = index(
			\ s:buffergator_catalog_sort_regimes, self.sort_regime)
		let l:cur_regime += 1
		if l:cur_regime < 0 || l:cur_regime >=
			\ len(s:buffergator_catalog_sort_regimes)
			let self.sort_regime =
				\ s:buffergator_catalog_sort_regimes[0]
		else
			let self.sort_regime =
				\ s:buffergator_catalog_sort_regimes[l:cur_regime]
		endif
		call self.open(1)
		let l:sort_desc = get(
			\ s:buffergator_catalog_sort_regime_desc,
			\ self.sort_regime, ["??", "in unspecified order"])[1]
		call s:_messenger.send_info("sorted " . l:sort_desc)
	endfunction
	...
	function! l:catalog_viewer.cycle_directory_path_display() dict
		if self.display_regime != "basename"
			call s:_messenger.send_info(
				\ "cycling full/relative directory paths only makes sense when using the 'basename' display regime")
			return
		endif
		if g:buffergator_show_full_directory_path
			let g:buffergator_show_full_directory_path = 0
			call s:_messenger.send_info(
				\ "displaying relative directory path")
			call self.open(1)
		else
			let g:buffergator_show_full_directory_path = 1
			call s:_messenger.send_info(
				\ "displaying full directory path")
			call self.open(1)
		endif
	endfunction
	...
	function! l:catalog_viewer.cycle_display_regime() dict
		let l:cur_regime = index(
			\ s:buffergator_catalog_display_regimes, self.display_regime)
		let l:cur_regime += 1
		if l:cur_regime < 0 || l:cur_regime >=
			\ len(s:buffergator_catalog_display_regimes)
			let self.display_regime =
				\ s:buffergator_catalog_display_regimes[0]
		else
			let self.display_regime =
				\ s:buffergator_catalog_display_regimes[l:cur_regime]
		endif
		let b:did_syntax = 0
		call self.open(1)
		let l:display_desc = get(
			\ s:buffergator_catalog_display_regime_desc,
			\ self.display_regime, ["??", "in unspecified order"])[1]
		call s:_messenger.send_info("displaying " . l:display_desc)
	endfunction

	" Cycles window regime.
	function! l:catalog_viewer.cycle_viewport_modes() dict
		let l:cur_mode = index(
			\ s:buffergator_viewport_split_modes_cycle_list,
			\ g:buffergator_viewport_split_policy)
		let l:cur_mode += 1
		if l:cur_mode < 0 || l:cur_mode >=
			\ len(s:buffergator_viewport_split_modes_cycle_list)
			let g:buffergator_viewport_split_policy =
				\ s:buffergator_viewport_split_modes_cycle_list[0]
		else
			let g:buffergator_viewport_split_policy =
				\ s:buffergator_viewport_split_modes_cycle_list[l:cur_mode]
		endif
	...
	function! l:catalog_viewer.cycle_autodismiss_modes() dict
		if (g:gt_autodismiss_on_select)
			let g:gt_autodismiss_on_select = 0
		call s:_messenger.send_info(
			\ "will stay open on selection (autodismiss-on-select: OFF)")
		else
			let g:gt_autodismiss_on_select = 1
		call s:_messenger.send_info(
			\ "will close on selection (autodismiss-on-select: ON)")
		endif
	endfunction
...
function! s:NewBufferCatalogViewer()
	...
	let l:catalog_viewer = s:NewCatalogViewer(
		\ "[[buffergator-buffers]]", "buffergator")
	...
	function! l:catalog_viewer.open(...) dict
		...
		if self.bufnum < 0 || !bufexists(self.bufnum)
			...
			call self.activate_viewport()
			call self.render_buffer()
			" if (a:0 > 0 && a:1) || b:gt_catalog_viewer != self
			"	  call self.render_buffer()
			" else
	...
	function! l:catalog_viewer.setup_buffer_syntax() dict
		if has("syntax") && (!exists('b:did_syntax') || !b:did_syntax)
			syntax clear
			syn region BuffergatorFileLine start='^'
				\ keepend oneline end='$'
			syn match BuffergatorBufferNr '^\[.\{3\}\]'
				\ containedin=BuffergatorFileLine
			let l:line_symbols = values(s:buffergator_buffer_line_symbols)
			execute "syn match BuffergatorSymbol
				\ '[" . join(l:line_symbols,"") . "]' containedin=BuffergatorFileLine"
			for l:buffer_status_index in range(0,
				\ len(s:buffergator_buffer_line_symbols_order) - 1)
			  ...
			  let l:pattern .= repeat('.',
				  \ len(s:buffergator_buffer_line_symbols_order)
				  \ - (l:buffer_status_index + 1))
			  let l:pattern .= '\s.\{-}/'
			  let l:pattern_name =
				  \ "Buffergator" . toupper(l:name[0]) . tolower(l:name[1:]) . "Entry"
			  let l:element = [
				\ "syn match",
	...
	function! l:catalog_viewer.setup_buffer_keymaps() dict
		...
		if !exists("g:buffergator_use_new_keymap") || !g:buffergator_use_new_keymap
			...
			"""" Catalog management
			noremap <buffer> <silent> cs :call b:gt_catalog_viewer.cycle_sort_regime()<CR>
			noremap <buffer> <silent> cd :call b:gt_catalog_viewer.cycle_display_regime()<CR>
			noremap <buffer> <silent> cp :call b:gt_catalog_viewer.cycle_directory_path_display()<CR>
			noremap <buffer> <silent> cw :call b:gt_catalog_viewer.cycle_viewport_modes()<CR>
			noremap <buffer> <silent> cq :call b:gt_catalog_viewer.cycle_autodismiss_modes()<CR>
			...
			noremap <buffer> <silent> d  :<C-U>call b:gt_catalog_viewer.delete_target(0, 0)<CR>
			noremap <buffer> <silent> D  :<C-U>call b:gt_catalog_viewer.delete_target(0, 1)<CR>
			noremap <buffer> <silent> x  :<C-U>call b:gt_catalog_viewer.delete_target(1, 0)<CR>
			...
			noremap <buffer> <silent> po :<C-U>call b:gt_catalog_viewer.visit_target(1, 0, "")<CR>
			noremap <buffer> <silent> ps :<C-U>call b:gt_catalog_viewer.visit_target(1, 0, "vert sb")<CR>
			noremap <buffer> <silent> pi :<C-U>call b:gt_catalog_viewer.visit_target(1, 0, "sb")<CR>
			...
			noremap <buffer> <silent> eo :<C-U>call b:gt_catalog_viewer.visit_open_target(0, !g:gt_autodismiss_on_select, "")<CR>
			noremap <buffer> <silent> es :<C-U>call b:gt_catalog_viewer.visit_open_target(0, !g:gt_autodismiss_on_select, "vert sb")<CR>
			noremap <buffer> <silent> ei :<C-U>call b:gt_catalog_viewer.visit_open_target(0, !g:gt_autodismiss_on_select, "sb")<CR>
			noremap <buffer> <silent> et :<C-U>call b:gt_catalog_viewer.visit_open_target(0, !g:gt_autodismiss_on_select, "tab sb")<CR>

		else

			"""" Catalog management
			noremap <buffer> <silent> s :call b:gt_catalog_viewer.cycle_sort_regime()<CR>
			noremap <buffer> <silent> i :call b:gt_catalog_viewer.cycle_display_regime()<CR>
			noremap <buffer> <silent> u :call b:gt_catalog_viewer.rebuild_catalog()<CR>
			noremap <buffer> <silent> q :call b:gt_catalog_viewer.close(1)<CR>
			noremap <buffer> <silent> d :call b:gt_catalog_viewer.delete_target(0, 0)<CR>
			noremap <buffer> <silent> D :call b:gt_catalog_viewer.delete_target(0, 1)<CR>
			noremap <buffer> <silent> x :call b:gt_catalog_viewer.delete_target(1, 0)<CR>
			noremap <buffer> <silent> X :call b:gt_catalog_viewer.delete_target(1, 1)<CR>
			...
			noremap <buffer> <silent> ws :call b:gt_catalog_viewer.visit_target(1, 0, "sb")<CR>
			noremap <buffer> <silent> wv :call b:gt_catalog_viewer.visit_target(1, 0, "vert sb")<CR>
			noremap <buffer> <silent> t  :call b:gt_catalog_viewer.visit_target(1, 0, "tab sb")<CR>

			" open target line in other window, closing catalog
			noremap <buffer> <silent> O  :call b:gt_catalog_viewer.visit_target(0, 0, "")<CR>
			noremap <buffer> <silent> wS :call b:gt_catalog_viewer.visit_target(0, 0, "sb")<CR>
			noremap <buffer> <silent> wV :call b:gt_catalog_viewer.visit_target(0, 0, "vert sb")<CR>
			noremap <buffer> <silent> T  :call b:gt_catalog_viewer.visit_target(0, 0, "tab sb")<CR>
	...
	function! l:catalog_viewer.render_buffer() dict
		...
		for l:bufinfo in self.buffers_catalog
			...
			if self.display_regime == "basename"
				let l:line .= s:_format_align_left(
					\ l:bufinfo.basename, self.max_buffer_basename_len, " ")
				let l:line .= "  "
			...
			elseif self.display_regime == "bufname"
				...
				throw s:_messenger.format_exception(
					\ "Invalid display regime: '" . self.display_regime . "'")
			endif
			call self.append_line(l:line, l:bufinfo.bufnum)
		endfor
		let b:gt_last_render_time = localtime()
		try
	...
	function! l:catalog_viewer.get_target_bufnum(cmd_count) dict
		if a:cmd_count == 0
			...
			if bufnr(l:jump_to_bufnum) == -1
				call s:_messenger.send_info(
					\ "Not a valid buffer number: " . string(l:jump_to_bufnum) )
				return -1
			endif
			...
			call s:_messenger.send_info(
				\ "Not a listed buffer number: " . string(l:jump_to_bufnum) )
			return -1
		endif
	endfunction

	" Go to the selected buffer.
	function! l:catalog_viewer.visit_target(
		\ keep_catalog, refocus_catalog, split_cmd) dict range
		let l:jump_to_bufnum = self.get_target_bufnum(v:count)
		if l:jump_to_bufnum == -1
			return 0
		endif
		let l:cur_tab_num = tabpagenr()
		if (self.split_mode != "buffer"
			\ || !empty(a:split_cmd)) && !a:keep_catalog
			call self.close(0)
		endif
		...
		call s:_messenger.send_info(
			\ expand(bufname(l:jump_to_bufnum)))
	endfunction

	" Go to the selected buffer, preferentially using a window that already is
	" showing it; if not, create a window using split_cmd
	function! l:catalog_viewer.visit_open_target(
		\ unconditional, keep_catalog, split_cmd) dict range
		let l:jump_to_bufnum = self.get_target_bufnum(v:count)
		if l:jump_to_bufnum == -1
			return 0
		endif
	...
	function! l:catalog_viewer.delete_target(wipe, force) dict range
		...
		if !bufexists(l:bufnum_to_delete)
			call s:_messenger.send_info(
				\ "Not a valid or existing buffer")
			return 0
		endif
		if a:wipe && a:force
		...
		"	  if bufexists(abufnum) && buflisted(abufnum)
		"	  \ && abufnum != l:bufnum_to_delete
		...
		"		  if bufexists(abufnum)
		"		  \ && buflisted(abufnum) && abufnum != l:bufnum_to_delete
		"			  let l:alternate_buffer = abufnum
		"			  break
		"		  endif
		"	  endfor
		...
		"	  call s:_messenger.send_warning(
		"	  \ "Cowardly refusing to delete last listed buffer")
		"	  return 0
		" endif
		...
		if len(self.buffers_catalog) == 1
			if self.buffers_catalog[0].bufnum == l:bufnum_to_delete
				...
				call s:_messenger.send_warning("Buffer not found")
				return 0
			endif
		endif
		let l:alternate_buffer = -1
				...
				if xbi == len(self.buffers_catalog)-1
					if xbi > 0
						let l:alternate_buffer =
							\ self.buffers_catalog[xbi-1].bufnum
					else
						call s:_messenger.send_warning(
							\ "Cowardly refusing to delete last listed buffer")
						return 0
		...
		for winnum in range(1, winnr('$'))
			...
			if wbufnum == l:bufnum_to_delete
				...
				execute("silent keepalt keepjumps buffer "
					\ . l:alternate_buffer)
			endif
		endfor
		...
		try
			...
			call s:_messenger.send_info(l:message)
		catch /E89/
			...
			let l:message = 'Failed to ' . l:operation_desc . ' "' . l:bufname
				\ . '" because it is modified; use unconditional version of this command to force operation'
			call s:_messenger.send_error(l:message)
		catch //
			for winnum in l:changed_win_bufs
				execute(winnum . "wincmd w")
				execute("silent keepalt keepjumps buffer " . l:bufnum_to_delete)
	...
	function! l:catalog_viewer.goto_index_entry(
		\ direction, visit_target, refocus_catalog) dict range
		if v:count > 0
			let l:target_bufnum = v:count
			if bufnr(l:target_bufnum) == -1
				call s:_messenger.send_info(
					\ "Not a valid buffer number: " . string(l:target_bufnum) )
				return -1
			endif
			let l:ok = 0
			for lnum in range(1, line("$"))
			...
			if !l:ok
				call s:_messenger.send_info(
					\ "Not a listed buffer number: " . string(l:target_bufnum) )
				return -1
			endif
		else
...
function! s:NewTabCatalogViewer()
	...
	function! l:catalog_viewer.render_buffer() dict
		...
		for l:tidx in range(len(self.tab_catalog))
			...
			for widx in range(len(l:tabinfo))
				...
				if self.display_regime == "basename"
					let l:subline .=
						\ s:_format_align_left(fnamemodify(l:tabbufname, ":t"), 30, " ")
	...
	function! l:catalog_viewer.setup_buffer_syntax() dict
		if has("syntax") && !(exists('b:did_syntax'))
			...
			" hi! link BuffergatorTabPageNumber Special
			" hi! link BuffergatorTabPageLineEnd Title
			hi! link BuffergatorCurrentEntry CursorLine
			let b:did_syntax = 1
		endif
	endfunction
	...
	function! l:catalog_viewer.visit_target() dict
		let l:cur_line = line(".")
		if !has_key(l:self.jump_map, l:cur_line)
			call s:_messenger.send_info("Not a valid navigation line")
			return 0
		endif
		...
		" call s:_messenger.send_info(expand(bufname(l:jump_to_bufnum)))
	endfunction
...
function! BuffergatorBuffersStatusLine()
	...
	if has_key(b:gt_catalog_viewer.jump_map, l:line)
		let l:status_line .= " Buffer " . string(l:line) . " of "
			\ . string(len(b:gt_catalog_viewer.buffers_catalog))
	endif
...
function! BuffergatorTabsStatusLine()
	...
	let l:line = line(".")
	if has_key(b:gt_catalog_viewer.jump_map, l:line)
		let l:status_line .= " Tab Page: "
			\ . b:gt_catalog_viewer.jump_map[l:line].target[0]
		let l:status_line .= ", Window: "
			\ . b:gt_catalog_viewer.jump_map[l:line].target[1]
	endif
	return l:status_line
endfunction
" 1}}}
...
function! buffergator#BuffergatorCycleMru(dir, bufopencmd)
	...
	if len(l:mru_cycle_list) < 2
		call s:_messenger.send_info("only one buffer available")
		return
	endif
	...
	if l:target_buf > 0
		...
		call s:_messenger.send_info("no previous/next existing buffers available")
	endif
endfunction
...
function! buffergator#UpdateBuffergator(event, affected)
	...
	let l:calling = bufnr("%")
	let l:self_call = 0
	let l:buffergators = s:_find_buffers_with_var("is_gt_buffer", 1)
	...
	if a:event == "delete"
		call filter(s:_catalog_viewer.buffers_catalog,
			\ 'v:val["bufnum"] != ' . a:affected)
	endif
	...
	for l:gator in l:buffergators
		if bufwinnr(l:gator) > 0
			...
			if has_key(b:gt_catalog_viewer, "tab_catalog")
				continue
			endif
	...
	if exists("b:is_gt_buffer") && !l:self_call
		try
			execute "wincmd p"
		catch //
		endtry
...
" ==============================================================================
if exists("s:_messenger")
	unlet s:_messenger
endif
let s:_messenger = s:NewMessenger("")
