...
function! BuffergatorUpdateMRU(acmd_bufnr)
	...
	if g:buffergator_track_mru
		...
		" \ || !(empty(getbufvar(bnum, "netrw_browser_active")))
		if bnum == 0 || !buflisted(bnum)
			\ || getbufvar(bnum, "&filetype") == "netrw"
			return
		endif
...
if !exists('g:buffergator_suppress_keymaps') || !g:buffergator_suppress_keymaps
	...
	" nnoremap <silent> <Esc> if exists("b:is_gt_buffer") <bar> :BuffergatorClose<CR> <bar> endif
	...
	if !exists('g:buffergator_suppress_mru_switching_keymaps')
		...
		if !exists('g:buffergator_keep_old_mru_switching_keymaps')
	...
	if !exists('g:buffergator_suppress_mru_switch_into_splits_keymaps')
		\ || !g:buffergator_suppress_mru_switch_into_splits_keymaps
